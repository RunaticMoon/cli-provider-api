...
def test_cancel_persisted_before_wrapper_call(env, tmp_path):
    ...
    wrapper run is cancelled; a review-state receipt keeps its truthful
    state but can never be accepted afterwards."""
    tid, _outcome, receipt = _dispatch_card(env, tmp_path)
    ...
    # The run genuinely reached review before cancel — the flag, not a
    # rewritten state, is what blocks the accept path (proved below).
    assert res.state == "review"
    with pytest.raises(ControlError, match="cancellation"):
        cmd_accept(store_path=env["store_path"], board_db=env["db"],
                   policy_path=_policy_path(env, tmp_path),
                   task_id=tid, actor="op-test",
                   integrated_revision=env["rev"])
    # And the card was never allowed to complete.
...
def test_approve_consumes_once_and_unblocks(env, tmp_path):
    ...
    out = cmd_approve(store_path=env["store_path"], board_db=env["db"],
                    policy_path=_policy_path(env, tmp_path),
                    approval_id=ap.approval_id, actor="op-test")
...
def test_accept_requires_review_proof(env, tmp_path):
    tid, _outcome, receipt = _dispatch_card(env, tmp_path)
    rev = env["rev"]
    out = cmd_accept(store_path=env["store_path"], board_db=env["db"],
                     policy_path=_policy_path(env, tmp_path),
                     task_id=tid, actor="op-test", integrated_revision=rev)
    assert out["task_status"] == "done"
    assert env["bridge"].call("get_task", task_id=tid)["task"]["status"] \
        == "done"
    # Integrated revision is on the closing run's metadata.
    meta = env["bridge"].call("get_task", task_id=tid)["task"][
        "last_run_metadata"]
    assert json.loads(meta)["integrated_revision"] == rev


def test_accept_rejects_non_review_card(env, tmp_path):
    tid = env["bridge"].call(
        "create_task", title="c", assignee="jev-native", body="x",
    )["task_id"]
    with pytest.raises(ControlError, match="no review-state"):
        cmd_accept(store_path=env["store_path"], board_db=env["db"],
                   policy_path=_policy_path(env, tmp_path),
                   task_id=tid, actor="op-test",
                   integrated_revision=env["rev"])
...
def test_accept_rejects_bad_revision(env, tmp_path):
    tid, _outcome, receipt = _dispatch_card(env, tmp_path)
    with pytest.raises(ControlError):
        cmd_accept(store_path=env["store_path"], board_db=env["db"],
                   policy_path=_policy_path(env, tmp_path),
                   task_id=tid, actor="op-test",
                   integrated_revision="notasha")


def test_accept_rejects_cancelled_run(env, tmp_path):
    tid, _outcome, receipt = _dispatch_card(env, tmp_path)
    cmd_cancel(store_path=env["store_path"], board_db=env["db"],
               policy_path=_policy_path(env, tmp_path),
               dispatch_id=receipt.dispatch_id, actor="op-test")
...
def test_resolve_unknown_completed_goes_to_review(env, tmp_path):
    ...
    # Simulate a crashed-after-submit: run exists, receipt unknown.
    run_id = env["stub"].submit_count  # force a run to exist via client
    ...
    out = cmd_resolve(store_path=env["store_path"], board_db=env["db"],
                      policy_path=_policy_path(env, tmp_path),
                      dispatch_id=res.dispatch_id, actor="op-test")
    assert out["resolved"] == "review"
    assert env["bridge"].call("get_task", task_id=tid)["task"]["status"] \
        == "review"


def test_resolve_unknown_no_run_aborts(env, tmp_path):
    """No wrapper run ever existed -> nothing executed -> abort so the card
    may dispatch again (the ONLY retryable unknown)."""
    data, rev = env["policy_data"], env["rev"]
    tid = env["bridge"].call(
        "create_task", title="c", assignee="jev-native", body="x",
    )["task_id"]
    ...
    out = cmd_resolve(store_path=env["store_path"], board_db=env["db"],
                      policy_path=_policy_path(env, tmp_path),
                      dispatch_id=res.dispatch_id, actor="op-test")
    assert out["resolved"] == "aborted"
    store = DispatchStore(env["store_path"])
    assert store.get(res.dispatch_id).state == "aborted"
    store.close()
